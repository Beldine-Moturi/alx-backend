...
class MRUCache(BaseCaching):
    """Defines methods that implement MRU Caching"""
    ...
    def put(self, key, item):
        ...
        if key and item:
            if len(self.cache_data) >= self.MAX_ITEMS:
                ...
                highest = max(self.access.values())
                keys = list(self.access.keys())
                h_key = keys[list(self.access.values()).index(highest)]
                # remove that key-value pair from cache
                print(f"DISCARD: {h_key}")
                self.cache_data.pop(h_key)
                self.access.pop(h_key)
